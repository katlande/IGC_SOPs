---
title: "Parse Alignment with Split-Pipe"
output: github_document
---

**This is an SOP for Parse-seq alignment on Cask written in May 2026. Newer versions of Split-pipe may be required going forward.**

# Setting up a Split-Pipe run
To align data from a parse experiment, you will need to set up a samp-list file based on the plate loading table, which looks like this:

![](~/Desktop/Example_ParseLoading.png)
The samp-list needs to assign wells to each sample, for example in the above, our samp-list would be:
```{bash, eval=FALSE}
cat > sample_list.txt
S1 A1:A6
S2 A7:A12
...
To run split-pipe, open the spipe_1.7 conda environment and update the $PATH:
```{bash, eval=FALSE}
...
New references need to be made when split-pipe is updated. Here is a mkref example for an updated version of the mm39 reference for split-pipe v1.7:
...
To load parse data into Seurat, load in samples one at a time with the following function:
